...
class CircleApp:
    ...
    def add_point(self):
        ...
        # If 3 points reached → change button label
        if len(self.points) == 3:
            self.btn_add.config(text="Generate Circle")
        else:
            self.btn_add.config(text="Add Point")
    ...
    def clear_all(self):
        self.points = []
        self.defaults_active = False
        self.btn_add.config(text="Add Point")  # reset label
        self.refresh_listbox()
        self.redraw()
