...
legend_elements = [
    ...
    # Initial Data Section (Expanded)
    plt.Line2D([0], [0], color='none', label=r"$\mathbf{Initial\ Data:}$"),
    plt.Line2D([0], [0], color='none', label=r"$\text{Bamboo\ Absorption:}\ 25\ \text{tons\ CO}_2\text{/acre/yr}$"),
    ...
    plt.Line2D([0], [0], color='none', label=r"$\mathbf{Conversation \ Bamboo\ Absorption:}$"),
  


# Conversion Formula: Acres to Square Miles

    plt.Line2D([0], [0], color='none', label=
        r"$\ 25\ \text{tons\ CO}_2\text{/acre/yr} \times 640\ \text{acres/sq\ mi} = 16,000\ \text{t/sq mi/yr}$"),
    ...
    plt.Line2D([0], [0], color='none', label=
        r"$\text{Land \%} = \frac{\text{Bamboo Area}}{\text{Land Area}} \times 100$"),
         ...
         plt.Line2D([0], [0], color='none', label=r"$\mathbf{.}$")
...
plt.legend(handles=legend_elements, loc='upper right', 
           bbox_to_anchor=(1.50, 1), frameon=True, fontsize=11,
...
# ======================
plt.tight_layout()
plt.savefig('v25-bamboo_co2_offset_corrected.jpg', dpi=300, bbox_inches='tight')  # Save high-res
plt.show()
