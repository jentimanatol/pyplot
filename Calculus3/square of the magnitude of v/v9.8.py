...
class VectorApp(tk.Tk):
    def __init__(self):
        ...
        self.geometry(f"{min(self.winfo_screenwidth()-100,1600)}x{min(self.winfo_screenheight()-150,1000)}")
        ...
        # --- Calculation buttons - now includes external window button!
        calcbtn_frm = tk.Frame(self, bg="#f8fcff")
        calcbtn_frm.grid(row=4, column=0, columnspan=7, sticky="ew", pady=(8,0))
        pad_x = (7,3); pad_y=(2,2)
        btnspecs = [
        ...
        self.calcbtns=[]
        for i, (text, cmd, color) in enumerate(btnspecs):
        ...
        # ----- NEW BUTTON: (external window for all vectors) -----
        b = tk.Button(
            calcbtn_frm, text="External window:\nVisualize all vectors",
            command=self.show_all_external,
            fg="white", bg="#1556a6",
            font=("Verdana", 13, "bold"), width=18, height=2
        )
        b.grid(row=0, column=len(btnspecs), padx=pad_x, pady=pad_y)
        self.calcbtns.append(b)
    ...
    def show_all_external(self):
        ...
        for vec, label, color, marker in vectors:
            ax.quiver(*origin, *vec, color=color, label=label, arrow_length_ratio=0.14, linewidth=2)
            ax.scatter([vec[0]], [vec[1]], [vec[2]], color=color, s=110, marker=marker)
            x, y, z = vec
            ax.plot([x, x], [y, y], [z, 0], linestyle='dashed', color=color, alpha=0.7)
            ax.plot([x, x], [y, 0], [0, 0], linestyle='dashed', color=color, alpha=0.5)
            ax.plot([x, 0], [0, 0], [0, 0], linestyle='dashed', color=color, alpha=0.4)
        ax.set_xlim([-lim, lim])
        ax.set_ylim([-lim, lim])
        ax.set_zlim([-lim, lim])
        ...
        ax.set_title("All Vectors (external 3D window)", fontsize=15)
        ax.grid(False)
